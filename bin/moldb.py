	...
	for i in range(NRec):
		...
		if tokens[0] == 'SWITCH':
			...
			if keyword=='LIST':
				...
				f=open(values[1]);
				file_values = [ s.strip() for s in f.read().splitlines()];
				f.close();
...
def addHash(a,b):
	if not 'MD5' in Params.params:
		return int(a*b % BIG_INT);
	else:
		return md5_hash( md5_hash(a) + md5_hash(b) )
...
def getHash(x):
	...
	if "MD5" in Params.params:
		S = md5_hash(s)

	elif "NewHash" in Params.params:
...
class HashDict(UserDict,Hashable):
	def getHash(self):
		...
		if "NewHash" or "MD5" in Params.params:
...
class ParametersBlock:
	...
	#	if "molecule" in param_names:
	#		print "Method"+self.name+"params[molecule]="+params["molecule"];
	...
	def parseLines(self,lines,currline):
		...
		if words[0]=='USE':
			...
			currline = nextLine;
                ...
                if not 'NewHash' and not "MD5" in Params.params:
			sys.stderr.write('WARNING: OldHash is used! (potentially wrong for omograms etc.). Consider adding "USE NewHash" or "USE MD5" after RUN METHOD statetemt\n')
